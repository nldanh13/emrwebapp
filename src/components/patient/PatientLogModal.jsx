    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: '10px 14px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontWeight: 600, fontSize: FS.md }}>Log session</span>
          ...
          {!loading && data && (<>
            {data.diagnostics && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: FS.xs, fontWeight: 600, color: C.text2, marginBottom: 6 }}>
                  Chẩn đoán hệ thống:
                </div>
                <pre style={{
                  fontSize: FS.xs, background: C.bg, padding: 8, borderRadius: 4,
            ...
            {data.files?.length > 0 && (
              <div style={{ marginBottom: 12 }}>
                ...
                {data.files.map(f => (
                  <div key={f.name} style={{ fontSize: FS.xs, color: C.text2, padding: '2px 0',
            ...
            {!data.files?.length && !data.scan_history && !data.activity_log && !data.diagnostics && (
              <div style={{ color: C.text3, fontSize: FS.sm }}>Chưa có log nào trong session này.</div>
            )}
          </>)}
        </div>
      </div>
